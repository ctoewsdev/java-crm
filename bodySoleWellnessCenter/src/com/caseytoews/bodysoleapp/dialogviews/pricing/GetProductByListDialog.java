...
public class GetProductByListDialog extends JDialog {
	...
	public GetProductByListDialog(JFrame frame, ProductDetailsDao productsDao) throws ApplicationException {
		allProducts = productsDao.getProductDetails();
		setBounds(100, 100, 600, 510);
		setTitle("Select A Product To Edit");
		setModal(true);
		setResizable(false);
		...
		{
			JLabel lblNewLabel = new JLabel("Click On A Product To Edit.");
		...
		serviceProducts.getSelectionModel().addListSelectionListener(new ListSelectionListener() {

			@Override
			public void valueChanged(ListSelectionEvent event) {
				if (!event.getValueIsAdjusting()) {
					ServicePricingListItem listProduct = serviceProducts.getSelectedValue();
					BodyServiceProduct product = listProduct.getProduct();
					GetProductByListDialog.this.dispose();
					try {
						EditServiceProductDialog dialog = new EditServiceProductDialog(frame, productsDao, product);
						dialog.setVisible(true);

					} catch (Exception e1) {
						LOG.error("EditServiceProductDialog() call in GetProductByListDialog() exception:" + e1);
		...
		packageProducts.getSelectionModel().addListSelectionListener(new ListSelectionListener() {

			@Override
			public void valueChanged(ListSelectionEvent event) {
				if (!event.getValueIsAdjusting()) {
					PackagedPricingListItem listProduct = packageProducts.getSelectedValue();
					BodyPackageProduct product = listProduct.getProduct();
					GetProductByListDialog.this.dispose();
					try {
						EditPackageProductDialog dialog = new EditPackageProductDialog(frame, productsDao, product);
						dialog.setVisible(true);

					} catch (Exception e1) {
						LOG.error("EditPackageProductDialog() call in GetProductByListDialog() exception:" + e1);
						UiController.handle(e1);
					}
					return;
				}
