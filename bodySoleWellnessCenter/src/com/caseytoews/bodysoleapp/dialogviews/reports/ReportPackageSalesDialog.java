...
public class ReportPackageSalesDialog extends JDialog {
	...
	public ReportPackageSalesDialog(JFrame frame, FilesReader filesReader) throws ApplicationException {
		setBounds(100, 100, 500, 350);
		setTitle("Monthly Sales: Packages");
		setModal(true);
		setResizable(false);
		...
		{
			JLabel lblNewLabel = new JLabel("Monthly Totals for Packages.");
		...
		}
		{
			...
			for (BodyPackage packaged : allPackaged) {

				LocalDate purchaseDate = LocalDate.parse(packaged.getPurchaseDate(), UiCommon.DATETIME_FORMAT_MMddyyyy);
				Report newReport = new Report(purchaseDate, packaged.getTotalGST(), packaged.getTotalCost(), 1);
...

		{
			...
			cancelButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					ReportPackageSalesDialog.this.dispose();
				}
			});
